...
func (cmd *SettleAuctionCommand) createOperation() (operation.Operation, error) {
	item := broker.NewSettleAuctionItemSingleNFT(cmd.n, cmd.Currency.CID)
	fact := broker.NewSettleAuctionFact([]byte(cmd.Token), cmd.sender, []broker.SettleAuctionItem{item})
